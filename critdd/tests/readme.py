...
class TestReadMe(TestCase):
    def test_readme(self):
        df = pd.read_csv(_URL).pivot(
            index="dataset_name", columns="classifier_name", values="accuracy"
        )
        diagram = Diagram(
            df.to_numpy(), treatment_names=df.columns, maximize_outcome=True
        )

        # check the content of the diagram
        expected_ranks = np.array([4.2, 3.75, 1.5, 3.5, 2.0])
        ...
        with open(
            os.path.splitext(output_path)[0] + ".tex", "r"
        ) as f:  # print the file contents
            print("\n" + "-" * 32, f.read(), "-" * 32 + "\n", sep="\n")
