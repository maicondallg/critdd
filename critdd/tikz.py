...
def to_file(path, tikz_code):
    ...
    if ext == ".svg":
        pdf_to_svg = subprocess.Popen(
            ["pdf2svg", root + ".pdf", root + ".svg"], stdout=subprocess.PIPE
        )
...
def to_str(
    ...
    *,
    ...
    axis_defaults = _merge_dicts(
        AXIS_OPTIONS,
        {  # diagram-dependent axis options
            "xmax": str(k),
    ...
    y_pos[sortperm] = np.concatenate(
        (
            2 + np.arange(changepoint) + (0.5 if k % 2 else 0),
            1 + np.arange(k - changepoint, 0, -1),
        )
    )
    anchors = np.array(["west", "east"])[
        (is_high if reverse_x else ~is_high).astype(int)
    ]
    ...
    for i in range(len(y_group_pos) - 1):
        if (
            len(np.intersect1d(groups[y_group_order[i + 1]], groups[y_group_order[i]]))
            > 0
        ):
            y_group_pos[y_group_order[i + 1]] = np.maximum(
                y_group_pos[y_group_order[i + 1]], y_group_pos[y_group_order[i]] + 0.2
...
def _environment(name, *contents, options=dict()):
    ...
    return "\n".join(
        [f"\\begin{{{name}}}" + options_str, *contents, f"\\end{{{name}}}"]
    )
